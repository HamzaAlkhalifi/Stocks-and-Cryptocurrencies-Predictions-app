...
def Yahoo1(ticker, days):
    ...
    if ticker != '' :
        try:
            ...
            data['EMA14'] = ta.trend.EMAIndicator(data['High'], window=14).ema_indicator()
            data['EMA28'] = ta.trend.EMAIndicator(data['High'], window=28).ema_indicator()

            # 2. RSI (Relative Strength Index)
            data['RSI_14'] = ta.momentum.RSIIndicator(data['High'], window=14).rsi()
            data['RSI_28'] = ta.momentum.RSIIndicator(data['High'], window=28).rsi()

            # 3. MACD (Moving Average Convergence Divergence)
            data['MACD'] = ta.trend.MACD(data['High']).macd()
            data['MACD Signal'] = ta.trend.MACD(data['High']).macd_signal()

            # 4. Bollinger Bands
            data['BB Upper'] = ta.volatility.bollinger_hband(data['High'])
            data['BB Middle'] =  ta.volatility.bollinger_mavg(data['High'])
            data['BB Lower'] = ta.volatility.bollinger_lband(data['High'])
            ...
            data['Prev_High2'] = data['High'].shift(2)
            data['Prev_High3'] = data['High'].shift(3)
            ...
            data = data[[
                ...
                'Close',
                'Low',
                'Prev_High3', "Prev_High2", 'Prev_High','High',
            ...
            df["TargetHigh"] = df["High"].shift(-days)
...
#----------------------------------------------------------------------------------------------------------------------------------------------------#
            for iter in range(2):
                ...
                # Features to predict 
                x = df.drop(['TargetHigh'],axis=1).values
                y = df["TargetHigh"].values.reshape(-1,1) # Reshape to 2D for scaling
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------------#
            ...
            with st.container(border=True):
                ...
                THigh = scaler_y.inverse_transform(y_test.detach().numpy())

                # Plotting differances
                fig, ax = plt.subplots()
                ax.plot(df.index[-x_test.shape[0]:], THigh, "-o", label=f'High after {days} days', alpha= 0.5)
                ax.plot(df.index[-x_test.shape[0]:], predictions, "-or", label=f'Predicted High after {days} days', alpha= 0.5)
                ax.plot(df.index[-x_test.shape[0]:], df.High[-x_test.shape[0]:], "-og", label='High today', alpha= 0.5)
                ax.legend()
                ...
                # Taking last days to predict the next days
                ...
                with st.expander(f"### Prediction of the next {days} Days:"):
                    for i in range(days):
                        st.write(f"Date: {pre_df.Date[i]}, Predicted High: {pre_df.Prediction[i]:,.2f}")
...
def Yahoo1_1(ticker, days):
    ...
    if ticker != '' :
        try:
            ...
            # 1. EMA (Exponential Moving Average)
            data['EMA14'] = ta.trend.EMAIndicator(data['Close'], window=14).ema_indicator()
            data['EMA28'] = ta.trend.EMAIndicator(data['Close'], window=28).ema_indicator()

            # 2. Bollinger Bands
            data['BB Upper'] = ta.volatility.bollinger_hband(data['Close'])
            data['BB Middle'] =  ta.volatility.bollinger_mavg(data['Close'])
            data['BB Lower'] = ta.volatility.bollinger_lband(data['Close'])
